...
export const ContentProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Assuming the API is available at /api/v1/webconfig/settings/ relative to the domain
    // or we might need to use the full URL if CORS allows.
    // For now, let's try relative path assuming proxy or same origin.
    const fetchSettings = async () => {
        try {
            // If template_id is present, we wait for parent to send content
    ...
    const handleMessage = (event: MessageEvent) => {
      // Security check: verify origin if possible, but for localhost dev we skip
  ...
  const updateContent = (key: string, value: string) => {
    setContent(prev => {
      ...
      // Send update to parent (SiteEditor)
