    ...
    const load = async () => {
      try {
        ...
        if (sres.ok && sdata) {
          ...
          if (sdata.company_description) setCompanyDescription(String(sdata.company_description));
        }
      } catch (e) {
          ...
          <div className="lg:col-span-2">
            <a href="/" className="inline-block">
              <span className="font-display text-3xl tracking-wider text-foreground">
                {companyName || "URBAN CARRY"}
              </span>
            </a>
            <p className="text-muted-foreground mt-4 max-w-sm">
              {companyDescription || "Diseñamos bolsos y cangureras para el hombre moderno que busca funcionalidad sin sacrificar el estilo."}
            </p>
